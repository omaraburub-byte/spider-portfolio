  ...
  return (
    <section className="py-16 bg-background">
      <div className="container mx-auto px-6">
        <h2 className="font-barrio text-5xl text-center mb-12 text-foreground">
          SPIDER ACHIEVEMENTS
        </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="bg-card border border-border rounded-xl p-6 text-center hover:border-spider-red transition-all duration-300 group hover:scale-105"
            >
              <div className={`inline-block p-3 rounded-full bg-background mb-4 ${stat.color} group-hover:bg-opacity-20`}>
                <stat.icon size={24} />
              </div>
