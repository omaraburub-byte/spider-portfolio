    ...
    <section id="experience" className="py-20 bg-black">
      <div className="container mx-auto px-6">
        ...
        <div className="relative max-w-4xl mx-auto">
          {/* Timeline line */}
          <div className="absolute left-8 md:left-1/2 transform md:-translate-x-1/2 h-full w-0.5 bg-spider-gray"></div>
          ...
          {experiences.map((exp, index) => (
            ...
            >
              ...
              <div className={`absolute left-6 md:left-1/2 transform md:-translate-x-1/2 w-4 h-4 rounded-full z-10 ${
                exp.type === 'work' ? 'bg-spider-red' : 'bg-spider-blue'
              } border-4 border-black`}></div>
              
              {/* Content card */}
              <div className={`md:w-5/12 ${
                index % 2 === 0 ? 'md:pr-12 ml-16 md:ml-0' : 'md:pl-12 ml-16 md:ml-0'
              }`}>
                <div className="bg-spider-gray border border-gray-800 rounded-xl p-6 hover:border-spider-red transition-all duration-300">
                  <div className="flex items-center mb-4">
                    <div className={`p-3 rounded-lg bg-black ${
                      exp.type === 'work' ? 'text-spider-red' : 'text-spider-blue'
                    } mr-4`}>
                      <exp.icon size={20} />
                    </div>
                    <div>
                      <h3 className="font-bold text-lg text-white">{exp.title}</h3>
                      <p className="text-spider-red">{exp.company}</p>
                    </div>
                  </div>
